...
export default function Layout() {
  ...
  const menu: {
  ...
  }[] = [
    {
      label: "Home",
      link: "/",
      icon: <HomeIcon className="size-7 sm:size-8" />,
      activeIcon: <HomeActiveIcon className="size-7 sm:size-8" />,
      active: location.pathname === "/",
    },
    {
      label: "Search",
      link: "/search",
      icon: <MagnifyingGlassIcon className="size-7 sm:size-8" />,
      activeIcon: <MagnifyingGlassIcon className="size-7 sm:size-8" />,
      active: location.pathname.includes("/search"),
    },
    {
      label: "Bookmark",
      link: "/bookmark",
      icon: <BookmarkIcon className="size-7 sm:size-8" />,
      activeIcon: <BookmarkActiveIcon className="size-7 sm:size-8" />,
      active: location.pathname.includes("/bookmark"),
    },
    {
      label: "Notification",
      link: "/notification",
      icon: <BellIcon className="size-7 sm:size-8" />,
      activeIcon: <BellActiveIcon className="size-7 sm:size-8" />,
      active: location.pathname.includes("/notification"),
    },
    {
      label: "Profile",
      link: "/profile",
      icon: <UserCircleIcon className="size-7 sm:size-8" />,
      activeIcon: <UserActiveIcon className="size-7 sm:size-8" />,
  ...
  return (
    <div className="h-screen w-screen sm:flex">
      <div className="fixed z-[999] bg-white bottom-0 max-md:left-0 max-md:right-0 px-2 py-5 sm:py-8 border-l border-neutral-300 flex gap-4 sm:static sm:flex-col max-xl:items-center sm:w-fit sm:h-full sm:px-6 xl:pr-20 xl:pl-12 ">
        <h1 className="max-xl:hidden text-2xl font-semibold text-neutral-700">
          netters.
        </h1>
        <div className="flex sm:flex-col max-xl:items-center max-sm:justify-evenly w-full sm:gap-8 sm:mt-28">
          {menu.map((m, i) => {
            return (
              <Link key={i} to={m.link} className="xl:flex items-center gap-3">
                <span
                ...
                <p
                  className={clsx(
                    "hidden xl:block",
                    m.active ? "text-black" : " text-neutral-600"
                  )}
                >
                  {m.label}
                </p>
